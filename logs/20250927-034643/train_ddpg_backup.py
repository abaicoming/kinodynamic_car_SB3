...
# LOG_DIR = os.path.join(LOG_DIR, "20250923-180006")
...
def rollout_one_episode(start_pose, max_steps=500):
    """返回 (xs, ys, start_state, goal_state, steps, is_success)"""
    # --- 你想要的起点/终点 ---
    # start_pose = (0.0, 0.0, 0.0)   # 起点 (x, y, theta)
    goal_pose  = (4.0, 4.0, 0.0)   # 终点 (xg, yg, thetag)
...
succ_cnt = 0
for i in range(N):
    xs, ys, st, gl, steps, ok = rollout_one_episode(start_pose = (0.0, i, 0.0))
